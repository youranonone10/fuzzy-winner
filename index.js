  ...
  while (true) {
    try {
      ...
      if (!res.data || !res.data.results) {
        console.warn("⚠️ No results or invalid response on page", page);
        break;
      }
      
      all = all.concat(res.data.results);
      console.log(`✅ Page ${page}: ${res.data.results.length} tickets fetched`);
      
      if (res.data.results.length < 30) break;
      page++;
...
async function getStats() {
  try {
    // Fetch only open & pending tickets (status 2 or 3)
    const open_tickets = await fetchAllPages("status:2 OR status:3");
    
    if (!open_tickets.length) {
      console.warn("⚠️ No open/pending tickets found");
      return {
        open: 0,
        unassigned: 0,
        assigned: 0,
    ...
    const open       = open_tickets.filter(t => t.status === 2);
    const pending    = open_tickets.filter(t => t.status === 3);
    const unassigned = open_tickets.filter(t => !t.responder_id);
    const assigned   = open_tickets.filter(t => !!t.responder_id);
    const overdue    = open_tickets.filter(t => t.due_by && new Date(t.due_by) < new Date() && t.status < 4);

    // unresolved = open + pending (not resolved/closed)
    const unresolved = open_tickets.filter(t => t.status < 4);

    console.log(`📈 Stats: Open=${open.length}, Unassigned=${unassigned.length}, Assigned=${assigned.length}, Pending=${pending.length}, Overdue=${overdue.length}`);
...
async function getAgentCounts() {
  try {
    ...
    const result = AGENT_IDS.map((id, i) => {
      const open = open_tickets.filter(t => t.status === 2 && t.responder_id === id).length;
      const unresolved = open_tickets.filter(t => t.status < 4 && t.responder_id === id).length;
    ...
    console.log("👥 Agent counts:", result);
    return result;
  } catch (err) {
    console.error("❌ getAgentCounts() error:", err.message);
    return AGENT_IDS.map((id, i) => ({
      id,
      name: AGENT_NAMES[i] || "Agent "+(i+1),
      count: 0,
...
async function assignTicket(ticketId, agentId) {
  try {
    ...
    console.log(`✅ Assigned ticket #${ticketId} to agent ${agentId}`);
    return true;
  } catch (err) {
...
async function runFullAssignment(agentIdList) {
  try {
    ...
    const all = await fetchAllPages("status:2 OR status:3");

    // Step 1: unassigned open tickets (status 2)
    ...
    console.log(`🎯 Full Assignment: ${unassigned.length} unassigned + ${unresolved.length} unresolved = ${toAssign.length} total`);
    
    if (toAssign.length === 0) {
      console.log("✅ Nothing to assign");
      return { assigned: 0, failed: 0, tickets: [], perAgent: {} };
    }
    ...
    console.log(`📤 Assignment complete: ${assigned} assigned, ${failed} failed`);
    return { assigned, failed, perAgent, tickets: results };
  } catch (err) {
    console.error("❌ runFullAssignment() error:", err.message);
    return { assigned: 0, failed: 0, tickets: [], perAgent: {} };
  }
}
...
async function runUnassignedOnly(agentIdList) {
  try {
    ...
    console.log(`🎯 Unassigned Only: ${unassigned.length} tickets to assign`);
    ...
    console.log(`📤 Unassigned assignment complete: ${assigned} assigned, ${failed} failed`);
    return { assigned, failed, perAgent, tickets: results };
  } catch (err) {
    console.error("❌ runUnassignedOnly() error:", err.message);
    return { assigned: 0, failed: 0, tickets: [], perAgent: {} };
  }
}
...
async function reshuffleAgent(agentId) {
  try {
    ...
    console.log(`🔄 Reshuffling ${tickets.length} tickets from agent ${agentId}`);
    ...
    console.log(`✅ Reshuffled: ${reassigned}/${tickets.length} tickets moved`);
    return { reassigned, total: tickets.length };
  } catch (err) {
    console.error("❌ reshuffleAgent() error:", err.message);
...
async function loadAll() {
  ...
  try {
    ...
    if (d.error) {
      ...
      toast("Failed to load stats — " + d.error, "error");
      return;
    }
    ...
    toast("Failed to load — check console", "error"); 
  }
...
async function loadAgents() {
  ...
  try {
    ...
    const d = await r.json();
    if (!d.length || d[0].error) {
      console.error("Agents API Error:", d[0]?.error);
      grid.innerHTML='<div class="empty">Failed to load agents — check logs</div>';
      return;
    }
    ...
    grid.innerHTML='<div class="empty">Failed to load agents</div>'; 
  }
}
...
async function assignAllNow() {
  ...
  try {
    ...
    console.error("assignAllNow error:", e);
...
async function assignSelectedUnassigned() {
  ...
  try {
    ...
    console.error("assignSelectedUnassigned error:", e);
    toast("Failed","error"); 
  }
...
async function assignSelectedFull() {
  ...
  try {
    ...
    console.error("assignSelectedFull error:", e);
    toast("Failed","error"); 
  }
...
async function doReshuffle(id, name) {
  ...
  try {
    ...
    console.error("doReshuffle error:", e);
    toast("Reshuffle failed","error"); 
  }
}
  ...
  if (p==="/api/assign-unassigned" && req.method==="POST") {
    ...
    req.on("end", async()=>{
      try { 
        ...
        console.error("❌ /api/assign-unassigned error:", e.message);
        res.writeHead(500); 
        res.end(JSON.stringify({error:e.message})); 
      }
    }); 
  ...
  if (p==="/api/assign-full" && req.method==="POST") {
    ...
    req.on("end", async()=>{
      try { 
        ...
        console.error("❌ /api/assign-full error:", e.message);
        res.writeHead(500); 
        res.end(JSON.stringify({error:e.message})); 
      }
    }); 
  ...
  if (p==="/api/reshuffle" && req.method==="POST") {
    ...
    req.on("end", async()=>{
      try { 
        ...
        console.error("❌ /api/reshuffle error:", e.message);
        res.writeHead(500); 
        res.end(JSON.stringify({error:e.message})); 
      }
    }); 
...
cron.schedule("0 4 * * 1-6", async ()=>{
  console.log("🚀 Cron: Running daily assignment at 9:30 AM IST");
  await runUnassignedOnly(AGENT_IDS);
...
cron.schedule("*/30 3-14 * * 1-6", async()=>{
  try {
    ...
    if(u.length>=20){ 
      console.log("🚨 Surge "+u.length+" unassigned tickets — auto-assigning"); 
      await runUnassignedOnly(AGENT_IDS); 
    }
  } catch(err) {
    console.error("❌ Cron surge check error:", err.message);
  }
}, {timezone:"Asia/Kolkata"});

console.log("🚀 TrustVA Ticket Desk is live!");
console.log("👥 "+AGENT_IDS.length+" agents configured");
console.log("⏰ Auto-assign enabled — 9:30 AM IST daily (Mon-Sat)");
